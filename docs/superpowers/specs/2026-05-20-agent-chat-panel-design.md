...
│  │  ├─ chat.service.ts     (OpenAI SDK 调用 Moonshot 兼容端点) │ │
...
3. 后端 `chat.service` 使用 **OpenAI SDK** 调用 Moonshot 兼容端点（`baseURL: https://api.moonshot.cn/v1`）
...
│   ├── chat.service.ts         # 核心：Moonshot API 调用 + Tool 编排
...
- 调用 OpenAI SDK (`openai.chat.completions.create` with `baseURL: https://api.moonshot.cn/v1`)
...
| 包名 | 用途 | 安装位置 |
|---|---|---|
| `openai` | OpenAI SDK 调用 Moonshot 兼容端点 | `apps/api` |
...
1. **Moonshot API 成本**：每次对话调用均计费，tool use 循环会增加 token 消耗
